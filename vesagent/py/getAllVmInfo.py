...
def PrintVmInfo(vm, depth=1):
   ...
   #singleinfo.update({"Path":summary.config.vmPathName})
   #singleinfo.update({"Guest":summary.config.guestFullName})
   #singleinfo.update({"Instance UUID":summary.config.instanceUuid})
   #singleinfo.update({"Heartbeatstatus":vm.guestHeartbeatStatus})
   #print (singleinfo)
   #allinfo.update(singleinfo)
...
def main():
   ...
   for child in content.rootFolder.childEntity:
      if hasattr(child, 'vmFolder'):
         ...
         print ("count of all vm's list")
	 print (len(vmList))
         for vm in vmList:
	    PrintVmInfo(vm)
	    #sinf = {}
            #sinf = PrintVmInfo(vm)
	    #print ("sinf RETURNED BY PRINTVMINFO()")
	    #print (sinf)
            #result = 'VM' + str(count)
   	    #allinfo[result] = sinf
   	    #if (sinf is not None):
		 #result = 'VM' + str(count)
   		 #allinfo[result] = sinf
   print("GETTING FINAL JSON FOR ALL VM DETAILS: ")
   print (listjson)
   json.dump(listjson, fp,sort_keys=True, indent=2)
   return 0
