...
class AAIClient(object):
        ...
        for network in content['networks']:
            resource = ("/cloud-infrastructure/cloud-regions/cloud-region/"
                        "%s/%s/oam-networks/oam-network/%s" % (
                            self.cloud_owner, self.cloud_region, network['id']))
            body = {
    ...
    def _del_networks(self, rsp):
        tenants = rsp.get("oam-networks", [])
        if not tenants:
            return
        for tenant in tenants["oam-network"]:
            resource = ("/cloud-infrastructure/cloud-regions/cloud-region/"
                        "%s/%s/oam-networks/oam-network/%s?resource-version=%s" % (
                            self.cloud_owner, self.cloud_region,
                            tenant['network-uuid'], tenant['resource-version']))
            resp = call_req(self.base_url, self.username, self.password,
                            rest_no_auth, resource, "DELETE",
                            headers=self.default_headers)
            if resp[0] != 0:
                raise VimDriverVioException(
                    ...
                    content="Failed to delete network %s: %s." % (
                        tenant['network-uuid'], resp[1]))

    def _del_azs(self, rsp):
        tenants = rsp.get("availability-zones", [])
        if not tenants:
            return
        for tenant in tenants["availability-zone"]:
            resource = ("/cloud-infrastructure/cloud-regions/cloud-region/"
                        "%s/%s/availability-zones/availability-zone/%s?"
                        "resource-version=%s" % (
                            self.cloud_owner, self.cloud_region,
                            tenant['availability-zone-name'],
            ...
            if resp[0] != 0:
                raise VimDriverVioException(
                    ...
                    content="Failed to delete availability zone %s: %s." % (
                        tenant['availability-zone-name'], resp[1]))
