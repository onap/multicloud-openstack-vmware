...
class AAIClient(object):
    ...
    def get_vim(self, get_all=False):
        ...
        if resp[0] != 0:
            raise VimDriverVioException(status_code=404, content=
    ...
    def update_identity_url(self):
        ...
        resource = ("/cloud-infrastructure/cloud-regions/cloud-region"
                    "/%s/%s" % (self.cloud_owner, self.cloud_region))
        resp = call_req(self.base_url, self.username, self.password,
                        rest_no_auth, resource, "PUT", content=json.dumps(vim)
                        headers=self.default_headers)
    ...
    def add_pservers(self, content):
        for hypervisor in content['hypervisors']:
            ...
            body = {
                ...
                'disk-in-gigabytes': hypervisor['local_gb'],
                'ram-in-megabytes': hypervisor['memory_mb'],
                # 'equip-type'
                # 'equip-vendor'
                # 'equip-model'
                # 'fqdn'
                ...
                # 'ipaddress-v6-oam'
                # 'inv-status'
                'pserver-id': hypervisor['service']['id'],
                # 'internet-topology'
            }
            resp = call_req(self.base_url, self.username, self.password,
                            rest_no_auth, resource, "PUT",
            ...
            resource = ("/cloud-infrastructure/pservers/pserver/%s/"
                        ...
                        hypervisor['hypervisor_hostname']))
            related-link = ("%s/cloud-infrastructure/cloud-regions/"
                            "cloud-region/%s/%s" % (
                self.base_url, self.cloud_owner, self.cloud_region))
            body = {
                ...
                'relationship-data': [
                    ...
                    },
                    {
                        ...
                        'relationship-value': hypervisor['hypervisor_hostname']
                    }
                ]
            }
            resp = call_req(self.base_url, self.username, self.password,
