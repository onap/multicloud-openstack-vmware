...
def _get_vim_full_path(raw_url):
    endpoint = re.findall('{(.*?)}', raw_url)
    if len(endpoint) != 1:
        raise Exception("%s is illegal. It is supposed to define only one "
                        "endpoint." % raw_url)

    endpoint = endpoint[0]
    # TODO(xiaohhui): replace endpoint with real endpoint.
    return re.sub('{(.*?)}', endpoint, raw_url)
...
def _build_api_controller(api_meta):
    ...
    # vim_path = _get_vim_full_path(path_meta["vim_path"])
