...
def handleDirectives(body):
    params = {}
    for oofd in body.get("oof_directives", []):
        if oofd.get("type", "") == "vnfc":
            for dd in oofd.get("directives", []):
                if dd.get("type", "") == "flavor_directive":
                    for attr in dd.get("attributes", []):
                        params[attr[
