...
class FakeNovaServer(APIView):
    ...
    def get(self,request,vimid,tenantid,serverid=None):


            token =  request.META.get("HTTP_X_AUTH_TOKEN","")
            if serverid:
                data = showServerDetail(token,tenantid=tenantid,serverid=serverid)
    ...
    def post(self,request,vimid,tenantid,serverid=None):
        ...
        token = request.META.get("HTTP_X_AUTH_TOKEN", "")
        if serverid:
            data = operator_server(token,serverid,action=create_req.keys()[0])
        if 'error' in data:
            return Response(data=data['error']['message'], status=data['error']['code'])
